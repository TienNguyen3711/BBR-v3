...
`pilot_recurrent_prioritized_ddqn.py` is a small technical screen. It uses the
legacy `FluidSimEnv` only as a proxy:
...
ablation. No legacy protocol alias is retained in the active release.
