...
class RecurrentPrioritizedClassicalDoubleDQN:
    """GRU + classical nonlinear core with the same masked Double-DQN update."""
    ...
    def __init__(
        ...
        if min(observation_dim, action_count, latent_dim) < 1:
            raise ValueError("observation_dim, action_count, and latent_dim must be positive")
        ...
        self.online_gru = torch.nn.GRU(observation_dim, latent_dim, batch_first=True)
        self.online_core = torch.nn.Sequential(
            torch.nn.Linear(latent_dim, latent_dim, bias=False), torch.nn.Tanh()
        )
        self.online_head = torch.nn.Linear(latent_dim, action_count)
        self.target_gru = torch.nn.GRU(observation_dim, latent_dim, batch_first=True)
        self.target_core = torch.nn.Sequential(
            torch.nn.Linear(latent_dim, latent_dim, bias=False), torch.nn.Tanh()
        )
        self.target_head = torch.nn.Linear(latent_dim, action_count)
        self.sync_target()
        self.optimizer = torch.optim.Adam(
            list(self.online_gru.parameters())
            + list(self.online_core.parameters())
    ...
    def optimize(self, batch_size: int = 16, importance_beta: float = 0.4) -> float | None:
        ...
        for item, weight in zip(batch, weights):
            ...
            losses.append(predicted.new_tensor(float(weight)) * torch.nn.functional.smooth_l1_loss(predicted, target))
    ...
    def sync_target(self) -> None:
        ...
        self.target_core.load_state_dict(self.online_core.state_dict())
        self.target_head.load_state_dict(self.online_head.state_dict())

    def param_count(self) -> int:
        return sum(
            parameter.numel()
            for module in (self.online_gru, self.online_core, self.online_head)
    ...
    def training_state_dict(self) -> dict:
        ...
        return {
            ...
            "online_core": self.online_core.state_dict(),
            "online_head": self.online_head.state_dict(),
            "target_gru": self.target_gru.state_dict(),
            "target_core": self.target_core.state_dict(),
    ...
    def load_training_state_dict(self, state: dict) -> None:
        ...
        for name in ("online_gru", "online_core", "online_head", "target_gru", "target_core", "target_head"):
