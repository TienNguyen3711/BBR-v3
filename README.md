...
**Status (2026-08-17):** all four RQs complete at full statistical power
...
qbbr/
  ...
  env/        fluid-model simulator (fluid_sim.py, fluid_env.py = Scenario A,
              multi_flow_env.py = Scenario B / coexistence)
...
Older checkpoints under `outputs/checkpoints/` (both cores) predate an
`actor_trunk`/`actor_heads` architecture refactor and are in a legacy
state-dict format; `MLPA2CAgent.load()` / `QA2CAgent.load()` both detect and
handle this format automatically (see `qbbr/tests/test_mlp_a2c.py` and
